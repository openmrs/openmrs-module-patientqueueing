...
public class CompletePatientQueueFragmentController {
	...
	public String post(@RequestParam("patientQueueId") Integer patientQueueId, Session session, UiUtils ui) throws Exception {
		PatientQueueingService patientService = Context.getService(PatientQueueingService.class);
		
		PatientQueue patientQueue = Context.getService(PatientQueueingService.class).getPatientQueueById(patientQueueId);
		
		patientService.completePatientQueue(patientQueue);
		
		return "redirect:"
		        + ui.pageLink("coreapps", "clinicianfacing/patient",
		            SimpleObject.create("patientId", patientQueue.getPatient().getId()));
	}
}
